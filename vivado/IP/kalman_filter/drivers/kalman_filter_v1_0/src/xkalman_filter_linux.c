...
typedef struct {
    ...
    XKalman_filter_uio_map maps[ MAX_UIO_MAPS ];
} XKalman_filter_uio_info;

/***************** Variable Definitions **************************************/
static XKalman_filter_uio_info uio_info;
...
static int uio_info_read_name(XKalman_filter_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XKalman_filter_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XKalman_filter_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XKalman_filter_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XKalman_filter_Initialize(XKalman_filter *InstancePtr, const char* InstanceName) {
	XKalman_filter_uio_info *InfoPtr = &uio_info;
