...
typedef struct {
    ...
    XDsp_alu_uio_map maps[ MAX_UIO_MAPS ];
} XDsp_alu_uio_info;

/***************** Variable Definitions **************************************/
static XDsp_alu_uio_info uio_info;
...
static int uio_info_read_name(XDsp_alu_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XDsp_alu_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XDsp_alu_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XDsp_alu_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XDsp_alu_Initialize(XDsp_alu *InstancePtr, const char* InstanceName) {
	XDsp_alu_uio_info *InfoPtr = &uio_info;
