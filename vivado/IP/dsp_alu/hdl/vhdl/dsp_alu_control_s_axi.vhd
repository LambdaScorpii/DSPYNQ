...
entity dsp_alu_control_s_axi is
generic (
...
end entity dsp_alu_control_s_axi;
...
architecture behave of dsp_alu_control_s_axi is
    ...
    constant ADDR_SEL_DATA_0  : INTEGER := 16#34#;
    constant ADDR_SEL_CTRL    : INTEGER := 16#38#;
    ...
    signal int_Sel             : UNSIGNED(1 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_SEL_DATA_0 =>
                        rdata_data <= RESIZE(int_Sel(1 downto 0), 32);
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_Sel(1 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_SEL_DATA_0) then
                    int_Sel(1 downto 0) <= (UNSIGNED(WDATA(1 downto 0)) and wmask(1 downto 0)) or ((not wmask(1 downto 0)) and int_Sel(1 downto 0));
                end if;
            end if;
        end if;
    end process;
